...
class GetAvailableServicesRs extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Canpar\StructType\GetAvailableServicesResult[]
     */
     ...
     * @param string $error
     * @param \Canpar\StructType\GetAvailableServicesResult[] $getAvailableServicesResult
     */
     ...
     * @param string $error
     * @return \Canpar\StructType\GetAvailableServicesRs
     */
     ...
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return \Canpar\StructType\GetAvailableServicesResult[]|null
     */
    ...
    public static function validateGetAvailableServicesResultForArrayConstraintsFromSetGetAvailableServicesResult(array $values = array())
    {
        ...
        foreach ($values as $getAvailableServicesRsGetAvailableServicesResultItem) {
            ...
            if (!$getAvailableServicesRsGetAvailableServicesResultItem instanceof \Canpar\StructType\GetAvailableServicesResult) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The getAvailableServicesResult property can only contain items of type \Canpar\StructType\GetAvailableServicesResult, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \Canpar\StructType\GetAvailableServicesResult $item
     * @return \Canpar\StructType\GetAvailableServicesRs
     */
    public function addToGetAvailableServicesResult(\Canpar\StructType\GetAvailableServicesResult $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Canpar\StructType\GetAvailableServicesResult) {
            throw new \InvalidArgumentException(sprintf('The getAvailableServicesResult property can only contain items of type \Canpar\StructType\GetAvailableServicesResult, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
