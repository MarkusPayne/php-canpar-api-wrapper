...
class SearchCanadaPostRs extends AbstractStructBase
{
    ...
    public static function validateAddressForArrayConstraintsFromSetAddress(array $values = array())
    {
        ...
        foreach ($values as $searchCanadaPostRsAddressItem) {
            ...
            if (!$searchCanadaPostRsAddressItem instanceof \Canpar\StructType\Address) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The address property can only contain items of type \Canpar\StructType\Address, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \Canpar\StructType\Address[] $address
     * @return \Canpar\StructType\SearchCanadaPostRs
     */
     ...
     * @throws \InvalidArgumentException
     * @param \Canpar\StructType\Address $item
     * @return \Canpar\StructType\SearchCanadaPostRs
     */
    public function addToAddress(\Canpar\StructType\Address $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Canpar\StructType\Address) {
            throw new \InvalidArgumentException(sprintf('The address property can only contain items of type \Canpar\StructType\Address, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->address[] = $item;
        return $this;
    }
