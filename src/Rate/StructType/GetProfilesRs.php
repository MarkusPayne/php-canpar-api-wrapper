...
class GetProfilesRs extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Canpar\Rate\StructType\Profile[]
     */
    public $profile;
    /**
     ...
     * @param string $error
     * @param \Canpar\Rate\StructType\Profile[] $profile
     */
     ...
     * @param string $error
     * @return \Canpar\Rate\StructType\GetProfilesRs
     */
     ...
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return \Canpar\Rate\StructType\Profile[]|null
     */
    ...
    public static function validateProfileForArrayConstraintsFromSetProfile(array $values = array())
    {
        ...
        foreach ($values as $getProfilesRsProfileItem) {
            ...
            if (!$getProfilesRsProfileItem instanceof \Canpar\Rate\StructType\Profile) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The profile property can only contain items of type \Canpar\Rate\StructType\Profile, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \Canpar\Rate\StructType\Profile $item
     * @return \Canpar\Rate\StructType\GetProfilesRs
     */
    public function addToProfile(\Canpar\Rate\StructType\Profile $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Canpar\Rate\StructType\Profile) {
            throw new \InvalidArgumentException(sprintf('The profile property can only contain items of type \Canpar\Rate\StructType\Profile, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->profile[] = $item;
        return $this;
    }
