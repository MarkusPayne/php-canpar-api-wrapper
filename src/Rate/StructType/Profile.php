...
class Profile extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Canpar\Rate\StructType\Service[]
     */
    public $services;
    /**
     * The shipper_num
     ...
     * @param \Canpar\Rate\StructType\Service[] $services
     * @param string $shipper_num
     * @param bool $validate_store_num
     * @param string $weight_unit
     */
    public function __construct(\Canpar\Rate\StructType\Address $address = null, $blind_manifest = null, $collect_service_allowed = null, $consolidation_type = null, $credit_cancelled = null, $manifest_processing_code = null, $otc = null, $pickup_allowed = null, array $services = array(), $shipper_num = null, $validate_store_num = null, $weight_unit = null)
    {
        $this
            ->setAddress($address)
     ...
     * @param \Canpar\Rate\StructType\Address $address
     * @return \Canpar\Rate\StructType\Profile
     */
    public function setAddress(\Canpar\Rate\StructType\Address $address = null)
    {
     ...
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return \Canpar\Rate\StructType\Service[]|null
     */
    ...
    public static function validateServicesForArrayConstraintsFromSetServices(array $values = array())
    {
        ...
        foreach ($values as $profileServicesItem) {
            ...
            if (!$profileServicesItem instanceof \Canpar\Rate\StructType\Service) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The services property can only contain items of type \Canpar\Rate\StructType\Service, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \Canpar\Rate\StructType\Service $item
     * @return \Canpar\Rate\StructType\Profile
     */
    public function addToServices(\Canpar\Rate\StructType\Service $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Canpar\Rate\StructType\Service) {
            throw new \InvalidArgumentException(sprintf('The services property can only contain items of type \Canpar\Rate\StructType\Service, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->services[] = $item;
        return $this;
    }
