...
class Proforma extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \Canpar\Rate\StructType\ProformaItem[]
     */
     ...
     * @param \Canpar\Rate\StructType\Address $broker_address
     ...
     * @param string $phone
     * @param \Canpar\Rate\StructType\ProformaItem[] $proforma_items
     * @param string $reason_for_export
     * @param string $reference
     * @param string $status
     * @param string $updated_on
     */
    public function __construct(\Canpar\Rate\StructType\Address $broker_address = null, $business_num = null, $currency_of_declared_value = null, $extension = null, $id = null, $inserted_on = null, $name = null, $permit_num = null, $phone = null, array $proforma_items = array(), $reason_for_export = null, $reference = null, $status = null, $updated_on = null)
    {
        $this
     ...
     * @param \Canpar\Rate\StructType\Address $broker_address
     * @return \Canpar\Rate\StructType\Proforma
     */
    public function setBroker_address(\Canpar\Rate\StructType\Address $broker_address = null)
    {
     ...
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return \Canpar\Rate\StructType\ProformaItem[]|null
     */
    ...
    public static function validateProforma_itemsForArrayConstraintsFromSetProforma_items(array $values = array())
    {
        ...
        foreach ($values as $proformaProforma_itemsItem) {
            ...
            if (!$proformaProforma_itemsItem instanceof \Canpar\Rate\StructType\ProformaItem) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The proforma_items property can only contain items of type \Canpar\Rate\StructType\ProformaItem, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \Canpar\Rate\StructType\ProformaItem $item
     * @return \Canpar\Rate\StructType\Proforma
     */
    public function addToProforma_items(\Canpar\Rate\StructType\ProformaItem $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Canpar\Rate\StructType\ProformaItem) {
            throw new \InvalidArgumentException(sprintf('The proforma_items property can only contain items of type \Canpar\Rate\StructType\ProformaItem, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
